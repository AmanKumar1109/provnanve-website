    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center px-4">
      ...
      >
        ...
        <div className="flex items-center gap-4 mb-5">
          {event.image ? (
            <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-xl overflow-hidden shrink-0 border border-white/10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {event.coordinators.map((coord, i) => (
                <div key={i} className="flex flex-col bg-white/5 border border-white/10 rounded-xl p-3">
                  ...
                  <span className="text-xs text-white/50">{coord.contact !== '—' ? coord.contact : 'N/A'}</span>
                </div>
              ))}
            </div>
          </div>

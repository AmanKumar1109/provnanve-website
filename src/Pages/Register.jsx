...
const useMountUnmount = (ref, visible) => {
  useEffect(() => {
    if (!ref.current) return;
    if (visible) {
      gsap.set(ref.current, { display: 'flex' });
      gsap.fromTo(ref.current, { opacity: 0 }, { opacity: 1, duration: 0.2, ease: 'power1.out' });
    } else {
      gsap.to(ref.current, {
        opacity: 0,
        duration: 0.2,
...
const CustomSelect = ({ label, name, value, options, onChange, icon: Icon, isSearchable = false }) => {
  ...
  const filteredOptions = options.map(group => {
    if (group.options) {
      const match = group.options.filter(opt =>
        opt.label.toLowerCase().includes(searchTerm.toLowerCase()) ||
        (opt.keywords && opt.keywords.toLowerCase().includes(searchTerm.toLowerCase()))
      );
      return match.length > 0 ? { ...group, options: match } : null;
    }
    return (
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      ...
      gsap.fromTo(el, { opacity: 0, y: 10, scale: 0.95 }, { opacity: 1, y: 0, scale: 1, duration: 0.2, ease: 'power2.out' });
    } else {
      gsap.to(el, {
    ...
    // Rotate chevron
    gsap.to(chevronRef.current, { rotate: isOpen ? 180 : 0, duration: 0.3, ease: 'power2.inOut' });
  }, [isOpen]);

  const close = () => { setIsOpen(false); setSearchTerm(''); setIsSearchOpen(false); };
    ...
    <div className="relative">
      ...
      >
        <div className="flex items-center gap-3">
          {Icon && (
            <Icon className={`w-5 h-5 transition-colors duration-300 ${isOpen ? 'text-purple-400' : 'text-purple-400/50 group-hover:text-purple-400'}`} />
          )}
            ...
            {!isSearchOpen ? (
              ...
              <div className="relative flex items-center gap-2">
                <input
                  ...
                  autoFocus
                />
                <button onClick={() => { setSearchTerm(''); setIsSearchOpen(false); }} className="p-2 text-white/30 hover:text-white transition-colors">
                  <X className="w-4 h-4" />
                </button>
              </div>
            )}
          ...
          {filteredOptions.length > 0 ? (
            filteredOptions.map((item, idx) => {
                ...
                return (
                  <div key={idx} className="mb-2 last:mb-0">
                    ...
                    {item.options.map(opt => (
                      <div
                        key={opt.value}
                        onClick={() => { onChange({ target: { name, value: opt.value } }); close(); }}
              ...
              return (
                <div
                  key={item.value}
                  onClick={() => { onChange({ target: { name, value: item.value } }); close(); }}
            ...
            <div className="px-6 py-8 text-center text-white/30 text-sm">No results found</div>
          )}
        </div>
      </div>
    </div>
...
const Register = () => {
  ...
  const [form, setForm] = useState({
    ...
    rollNumber: '', collegeType: 'within', collegeName: '',
    branch: '', year: '', tshirtSize: '',
    paymentApp: '', otherPaymentApp: '', transactionId: '',
  });
  ...
  // Card entrance
  useGsapFadeIn(cardRef, []);

  // Error banner slide-in
  useEffect(() => {
    if (!errorRef.current) return;
    if (error) {
      gsap.set(errorRef.current, { display: 'flex' });
      gsap.fromTo(errorRef.current, { opacity: 0, y: -8 }, { opacity: 1, y: 0, duration: 0.25, ease: 'power2.out' });
    } else {
      gsap.to(errorRef.current, {
        opacity: 0, duration: 0.15,
        onComplete: () => gsap.set(errorRef.current, { display: 'none' }),
      });
    }
  }, [error]);
  ...
  useEffect(() => {
    if (success && successRef.current) {
      gsap.fromTo(successRef.current, { opacity: 0, scale: 0.8 }, { opacity: 1, scale: 1, duration: 0.4, ease: 'back.out(1.4)' });
    }
  }, [success]);

  // Help modal
  useMountUnmount(helpRef, showHelp);
  useEffect(() => {
    if (!helpPanelRef.current) return;
    if (showHelp) {
      gsap.fromTo(helpPanelRef.current, { opacity: 0, scale: 0.9 }, { opacity: 1, scale: 1, duration: 0.25, ease: 'back.out(1.4)' });
    } else {
      gsap.to(helpPanelRef.current, { opacity: 0, scale: 0.9, duration: 0.15, ease: 'power2.in' });
    }
  }, [showHelp]);

  // Submit button hover / tap
  const onSubmitEnter = () => { if (!loading) gsap.to(submitBtnRef.current, { scale: 1.02, boxShadow: '0 0 30px rgba(124,58,237,0.4)', duration: 0.2 }); };
  const onSubmitLeave = () => { if (!loading) gsap.to(submitBtnRef.current, { scale: 1, boxShadow: '0 0 20px rgba(124,58,237,0.3)', duration: 0.2 }); };
  const onSubmitDown = () => { if (!loading) gsap.to(submitBtnRef.current, { scale: 0.98, duration: 0.1 }); };
  const onSubmitUp = () => { if (!loading) gsap.to(submitBtnRef.current, { scale: 1.02, duration: 0.1 }); };
  ...
  const handleSubmit = async e => {
    ...
    const isWithin = form.collegeType === 'within';
    const hasRequired = form.name && form.email && form.password && form.mobile && form.branch && form.year;
    const hasTshirt = isWithin ? form.tshirtSize : true;
    const hasConditional = isWithin ? form.rollNumber : form.collegeName;
    const hasPayment = isWithin
      ? (form.paymentApp && form.transactionId && (form.paymentApp !== 'other' || form.otherPaymentApp))
      : true;

    if (!hasRequired || !hasTshirt || !hasConditional || !hasPayment) {
    ...
    try {
      ...
      await setDoc(doc(db, 'users', user.uid), {
        ...
        rollNumber: isWithin ? form.rollNumber : '',
        collegeName: isWithin ? 'RVSCET' : form.collegeName,
        branch: form.branch,
        year: form.year,
        tshirtSize: isWithin ? form.tshirtSize : 'N/A',
        paymentApp: isWithin ? (form.paymentApp === 'other' ? form.otherPaymentApp : form.paymentApp) : 'N/A',
        transactionId: isWithin ? form.transactionId : 'N/A',
        paymentStatus: isWithin ? 'pending' : 'outside-college',
      ...
      switch (err.code) {
        case 'auth/email-already-in-use': setError('This email is already registered. Please login instead.'); break;
        ...
        default: setError(err.message || 'Registration failed. Please try again.');
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="relative w-full p-6 py-24 bg-black">
      ...
      {/* Card */}
      <div ref={cardRef} style={{ opacity: 0 }} className="relative z-10 w-full max-w-2xl mx-auto">
        <div className="glass-panel p-8 md:p-12 rounded-[2rem] border-purple-500/20 shadow-[0_0_50px_rgba(124,58,237,0.15)]">

          {/* ── Success State ── */}
          {success ? (
            <div ref={successRef} style={{ opacity: 0 }} className="flex flex-col items-center justify-center gap-6 py-12 text-center">
            ...
            <>
              ...
              <div className="text-center mb-8">
                ...
                <p className="mt-4 text-sm text-white/70 max-w-xl mx-auto">
                  Already registered?{' '}
                  <Link to="/login" className="text-purple-300 hover:text-purple-100 underline transition-colors">Login here</Link>{' '}
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <CustomSelect label="Select College Type" name="collegeType" value={form.collegeType} onChange={handleChange} icon={GraduationCap}
                    options={[
                ...
                <CustomSelect label="Select Branch" name="branch" value={form.branch} onChange={handleChange} icon={GraduationCap} isSearchable
                  options={[
                ...
                <CustomSelect label="Select Year" name="year" value={form.year} onChange={handleChange} icon={GraduationCap}
                  options={[
                ...
                {form.collegeType === 'within' && (
                  <CustomSelect label="Select T-Shirt Size" name="tshirtSize" value={form.tshirtSize} onChange={handleChange} icon={Shirt}
                    options={[
                    ...
                    <div className="flex flex-col items-center justify-center space-y-4 py-4 bg-white/5 rounded-2xl border border-white/10">
                      ...
                      <div className="flex flex-col items-center gap-1 pt-2">
                        ...
                        <p className="text-xs text-yellow-400/80 flex items-center gap-1 mt-1 text-center px-4">
                          <span>🎁</span>
                          <span>This Registration fee includes a <span className="font-bold text-yellow-300">Welcome Kit & T-Shirt</span></span>
                        </p>
                      </div>
                    </div>

                    <div className="space-y-6">
                      <CustomSelect label="Select Payment App Used" name="paymentApp" value={form.paymentApp} onChange={handleChange} icon={Wallet}
                        options={[
                      ...
                      <div className="relative group">
                        <Hash className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-purple-400/50 group-focus-within:text-purple-400 transition-colors" />
                        <input name="transactionId" type="text" placeholder="Enter Transaction ID (e.g. TXN123456...)" value={form.transactionId} onChange={handleChange} className={inputClass} />
                        <div className="flex flex-col mt-2 px-1">
                          <span className="text-[10px] text-white/40 italic">You can find this in your payment app history</span>
                          <button type="button" onClick={() => setShowHelp(true)} className="text-[11px] text-purple-400 hover:text-purple-300 underline mt-1 text-left flex items-center gap-1 transition-colors">
                ...
                {/* ── Help Modal ── */}
                <div ref={helpRef} style={{ display: 'none', opacity: 0 }} className="fixed inset-0 z-[100] items-center justify-center p-6 bg-black/80 backdrop-blur-sm">
                  <div ref={helpPanelRef} className="bg-[#1a0b2e] border border-purple-500/30 rounded-3xl p-8 max-w-md w-full relative shadow-[0_0_50px_rgba(124,58,237,0.2)]">
                    <button onClick={() => setShowHelp(false)} className="absolute top-4 right-4 text-white/40 hover:text-white p-2 rounded-full hover:bg-white/5 transition-all">
                      <X className="w-6 h-6" />
                    </button>
